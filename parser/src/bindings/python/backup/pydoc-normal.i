%feature("docstring") loadFile "
 Load a file of any format libPhrasedml knows about (potentially
 Phrasedml, SBML, or CellML).  If all attempts fail, the errors from
 the attempt to read the file in the Phrasedml format are saved, so if
 ...
 it as Phrasedml, and a '-1' is returned.

 NOTE:  This function will not attempt to parse the file as SBML if
 libPhrasedml is compiled with the '-NSBML' flag, and will not attempt
...
%feature("docstring") loadString "
 Load a string of any format libPhrasedml knows about (potentially
 Phrasedml, SBML, or CellML).  The first attempts to read the string as
 SBML, and if this results in an error, then reads it as Phrasedml.  If
 ...
 Parameter 'model' is The model, in (potentially) Phrasedml, SBML, or
 CellML format.
...
%feature("docstring") loadPhrasedmlFile "
 Loads a file and parses it as an Phrasedml file.  On an error, the
...
%feature("docstring") loadPhrasedmlString "
 Loads a string and parses it as an Phrasedml set of modules.  On an
 ...
 Parameter 'model' is The model in Phrasedml format.
...
%feature("docstring") loadSBMLFile "
 ...
 NOTE:  This function is unavailable when libPhrasedml is compiled with
 the '-NSBML' flag.
...
%feature("docstring") loadSBMLString "
 ...
 stored.  NOTE:  This function is unavailable when libPhrasedml is
...
%feature("docstring") loadSBMLStringWithLocation "
 ...
 is unavailable when libPhrasedml is compiled with the '-NSBML' flag.
...
%feature("docstring") addDirectory "
 ...
 look for a '.phrasedml' file (which contains rules about where to look
 locally for imported phrasedml and sbml files).
";
...
%feature("docstring") writePhrasedmlFile "
 Writes out an phrasedml-formatted file containing the given module.
 ...
 are written out as well, also in the phrasedml format.  Returns 0 on
 failure (and sets an error), 1 on success.
";


%feature("docstring") getPhrasedmlString "
 Returns the same output as writePhrasedmlFile, but to a char array
...
%feature("docstring") writeSBMLFile "
 ...
 libPhrasedml is compiled with the '-NSBML' flag.
...
%feature("docstring") getSBMLString "
 ...
 (rare).'  NOTE:  This function is unavailable when libPhrasedml is
 compiled with the '-NSBML' flag.
...
%feature("docstring") writeCompSBMLFile "
 ...
 'Hierarchichal Model Composition' package.  This retains Phrasedml's
 modularity in the SBML format.  Returns the output of libSBML's
 'writeSBML', which 'Returns non-zero on success and zero if the
 filename could not be opened for writing.'  An error indicating this
 is set on returning zero.  NOTE:  This function is unavailable when
 libPhrasedml is compiled with the '-NSBML' flag, or if compiled
...
%feature("docstring") getCompSBMLString "
 ...
 This retains Phrasedml's modularity in the SBML format.  Returns the
 output of libSBML's 'writeSBMLToString', which 'Returns the string on
 success and None if one of the underlying parser components fail
 (rare).'  NOTE:  This function is unavailable when libPhrasedml is
...
%feature("docstring") writeCellMLFile "
 ...
 retaining the same Phrasedml hierarchy using the CellML 'component'
 hieararchy.  Returns one on success and zero on failure.  NOTE:  This
 function is unavailable when libPhrasedml is compiled with the
 '-NCELLML' flag.
...
%feature("docstring") getCellMLString "
 ...
 This function is unavailable when libPhrasedml is compiled with the
 '-NCELLML' flag.
...
%feature("docstring") printAllDataFor "
 ...
 functions defined here in phrasedml_api.h.
";
...
%feature("docstring") getWarnings "
 ...
 generated by problems discovered in '.phrasedml' files.
";


%feature("docstring") getSBMLInfoMessages "
 Returns the 'info' messages from libSBML. libPhrasedml always
 translates its modules into SBML to check for errors.  If SBML finds
 errors, libPhrasedml gives up, passes on the error message, and does
 ...
 Phrasedml is unitless, for example, you will always find warnings
 ...
 empty string.  NOTE:  This function is unavailable when libPhrasedml
...
%feature("docstring") getMainModuleName "
 Returns the 'main' module name.  In Phrasedml, this is either the
...
%feature("docstring") getSymbolCompartmentsOfType "
 ...
 Rules about compartments in Phrasedml can be found in the Tutorial.pdf
...
%feature("docstring") getCompartmentForSymbol "
 ...
 In phrasedml, all symbols may have compartments, not just species.  If
...
%feature("docstring") getInteractionDividers "
 ...
 The length of the array can be obtained with 'getNumInteractions'.
";
...
%feature("docstring") getNumDNAStrands "
 ...
 the Phrasedml documentation (or, the number of physical cassettes of
...
%feature("docstring") freeAll "
 ...
 program.  If you freeanything yourself, however, calling this
...
%feature("docstring") OStream "
 ...
 text string output.  An example is the SBMLDocument.printErrors()
...
%feature("docstring") OFStream "
 ...
 The C++ ofstream ('output file stream') provides an  interface for
 writing data to files as output streams.  This class,  OFStream,
 wraps the C++ ofstream and provides an OStream  interface to it.  The
 file to be associated with the stream can be  specified as a
 parameter to the constructors in this class.

 This class may be useful because some libSBML methods accept an
 argument  for indicating where to send text string output.  An
 example is the  SBMLDocument.printErrors() method.  The methods use
 C++ style streams  and not Java stream objects.  The OStream object
 exists to bridge the  Java and underlying native implementation.  It
 is a simple wrapper  around the underlying stream object and provides
 a few basic methods for  manipulating it.
";
...
%feature("docstring") OStringStream "
 ...
 The C++ ostringstream ('output string stream class')  provides an
 interface to manipulating strings as if they were output  streams.
 This class class, OStringStream, wraps the  ostringstream and
 provides an OStream interface to it.
...
%feature("docstring") OStream::OStream "
 Creates a new OStream object with one of standard output stream
 objects.

 Parameter 'sot' is a value from the StdOSType enumeration(COUT, CERR,
 or CLOG)   indicating the type of stream to create.
";
...
%feature("docstring") OFStream::OFStream "
 ...
 This opens the given file 'filename' with the 'is_append' flag
 (default is false), and creates an OFStream object  instance that
 associates the file's content with an OStream object.

 Parameter 'filename' is the name of the file to open  Parameter
 'is_append' is whether to open the file for appending (default:
 false, meaning overwrite the content instead)
";
...
%feature("docstring") OFStream::open "
 ...
 This method opens a given file 'filename' with the given  'is_append'
 flag (whose default value is false),  and associates this stream
 object with the file's content.
