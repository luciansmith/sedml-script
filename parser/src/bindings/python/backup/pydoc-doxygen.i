%feature("docstring") loadFile "
 Load a file of any format libPhrasedml knows about (potentially Phrasedml, SBML, or CellML).  If all attempts fail, the errors from the attempt to read the file in the Phrasedml format are saved, so if the file is actually SBML or CellML, the error is likely to be \'but contains errors, the reported errors will be from the attempt to read it as Phrasedml, and a \'-1\' is returned.

 NOTE:  This function will not attempt to parse the file as SBML if libPhrasedml is compiled with the \'-NSBML\' flag, and will not attempt to parse the file as CellML if compiled with the \'-NCELLML\' flag.
...
%feature("docstring") loadString "
 Load a string of any format libPhrasedml knows about (potentially Phrasedml, SBML, or CellML).  The first attempts to read the string as SBML, and if this results in an error, then reads it as Phrasedml.  If this, too, results in an error, the second error is saved, and a \'-1\' is returned.

 NOTE:  This function will not attempt to parse the string as SBML if libPhrasedml is compiled with the \'-NSBML\' flag, and will not attempt to parse the string as CellML if compiled with the \'-NCELLML\' flag.
 ...
 @param model The model, in (potentially) Phrasedml, SBML, or CellML format.

 @see getLastError()
";


%feature("docstring") loadPhrasedmlFile "
 Loads a file and parses it as an Phrasedml file.  On an error, the error is saved, -1 is returned, and no information is stored.
...
%feature("docstring") loadPhrasedmlString "
 Loads a string and parses it as an Phrasedml set of modules.  On an error, the error is saved, -1 is returned, and no information is stored.

 @return a long integer indicating the index of the string read and stored.  On an error, returns -1 and no information is stored.

 @param model The model in Phrasedml format.
...
%feature("docstring") loadSBMLFile "
 ...
 @return a long integer indicating the index of the file read and stored.  On an error, returns -1 and no information is stored.
 NOTE:  This function is unavailable when libPhrasedml is compiled with the \'-NSBML\' flag.
...
%feature("docstring") loadCellMLFile "
 ...
 @return a long integer indicating the index of the file read and stored.  On an error, returns -1 and no information is stored.
 NOTE:  This function is unavailable when libPhrasedml is compiled with the \'-NCELLML\' flag.
...
%feature("docstring") addDirectory "
 Add a directory in which imported files may be found, and in which to look for a \'.phrasedml\' file (which contains rules about where to look locally for imported phrasedml and sbml files).
";
...
%feature("docstring") writePhrasedmlFile "
 Writes out an phrasedml-formatted file containing the given module.  If no module name is given, all modules in the current set are returned.  If the module depends on any sub-modules, those modules are written out as well, also in the phrasedml format.  Returns 0 on failure (and sets an error), 1 on success.
";


%feature("docstring") getPhrasedmlString "
 Returns the same output as writePhrasedmlFile, but to a char array instead of to a file.  Returns None on failure, and sets an error.
";
...
%feature("docstring") writeCompSBMLFile "
 Writes out a SBML-formatted XML file to the file indicated, using the \'Hierarchichal Model Composition\' package.  This retains Phrasedml\'s modularity in the SBML format.  Returns the output of libSBML\'s \'writeSBML\', which \'Returns non-zero on success and zero if the filename could not be opened for writing.\'  An error indicating this is set on returning zero.
 NOTE:  This function is unavailable when libPhrasedml is compiled with the \'-NSBML\' flag, or if compiled without the USE_COMP flag.
...
%feature("docstring") getCompSBMLString "
 Returns the same output as writeSBMLFile, but to a char array instead of to a file, using the \'Hierarchichal Model Composition\' package.  This retains Phrasedml\'s modularity in the SBML format.  Returns the output of libSBML\'s \'writeSBMLToString\', which \'Returns the string on success and None if one of the underlying parser components fail (rare).\'
 NOTE:  This function is unavailable when libPhrasedml is compiled with the \'-NSBML\' flag, or if compiled without the USE_COMP flag.
...
%feature("docstring") writeCellMLFile "
 Writes out a CellML-formatted XML file to the file indicated, retaining the same Phrasedml hierarchy using the CellML \'component\' hieararchy.  Returns one on success and zero on failure.
 NOTE:  This function is unavailable when libPhrasedml is compiled with the \'-NCELLML\' flag.
...
%feature("docstring") printAllDataFor "
 An example function that will print to stdout all the information in the given module.  This function probably isn\'t as useful to call as it is to examine and copy for your own purposes:  it only calls functions defined here in phrasedml_api.h.
";
...
%feature("docstring") getWarnings "
 When translating some other format to Phrasedml, elements that are unable to be translated are saved as warnings, retrievable with this function (returns None if no warnings present).  Warnings may also be generated by problems discovered in \'.phrasedml\' files.
";


%feature("docstring") getSBMLInfoMessages "
 Returns the \'info\' messages from libSBML. libPhrasedml always translates its modules into SBML to check for errors.  If SBML finds errors, libPhrasedml gives up, passes on the error message, and does not save the model.  However, libSBML may discover other things about your model it wants to tell you about, in \'info\' and \'warning\' messages.  Info messages are just things it found it thinks you might want to know; warning messages are things it found which it feels violates \'best practices\' in biological modelling, but not to the extent that it feels you did something actually wrong.  Since Phrasedml is unitless, for example, you will always find warnings about how you didn\'t set any units.  This function returns the \'info\' messages from libSBML.  If there are no info messages, returns an empty string.
 NOTE:  This function is unavailable when libPhrasedml is compiled with the \'-NSBML\' flag.
 @see getSBMLWarnings
";
...
%feature("docstring") getSymbolCompartmentsOfType "
 Returns the compartments associated with the symbols of the given return type.  Note that unlike in SBML, any symbol of any type may have an associated compartment, including compartments themselves.  Rules about compartments in Phrasedml can be found in the <A class=\'el\' target=\'_top\' HREF=\'Tutorial.pdf\'>Tutorial.pdf</a> document included with this documentation.
";
...
%feature("docstring") getCompartmentForSymbol "
 Returns the name of the compartment the given symbol is a member of.  In phrasedml, all symbols may have compartments, not just species.  If a symbol has no set compartment, and is not a member of a symbol with a set compartment, this will return \'default_compartment\'
";
...
%feature("docstring") getNumDNAStrands "
 Returns the number of unique DNA strands in the module, as defined in the Phrasedml documentation (or, the number of physical cassettes of DNA present in the module).  Useful in looping over the arrays returned by functions below.
";
